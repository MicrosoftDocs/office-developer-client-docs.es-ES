...
Por ejemplo, supongamos que la cadena de comando de cliente es "CustomerByID(4)", el encabezado de sección SQL es SQL CustomerByID y la nueva cadena de sección SQL es \[ \] "SELECT \* FROM Customers WHERE CustomerID = ?". El controlador generará , el encabezado de sección SQL es SQL CustomerByID y la nueva cadena de sección SQL es \[ \] "SELECT \* FROM Customers WHERE CustomerID = ?". El controlador generará "SELECT \* FROM Customers WHERE CustomerID = 4" y usará esa cadena para consultar el origen de datos.
