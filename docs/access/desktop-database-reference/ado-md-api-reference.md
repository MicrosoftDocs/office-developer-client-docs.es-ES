---
title: Referencia de API de ADO MD
TOCTitle: ADO MD API reference
ms:assetid: 897193ef-5b7c-78b2-cd88-92d19247fbaf
ms:mtpsurl: https://msdn.microsoft.com/library/JJ249602(v=office.15)
ms:contentKeyID: 48546163
ms.date: 09/18/2015
mtps_version: v=office.15
localization_priority: Normal
ms.openlocfilehash: fa3221814839200117bbc5bc824812437d68281f
ms.sourcegitcommit: 8fe462c32b91c87911942c188f3445e85a54137c
ms.translationtype: MT
ms.contentlocale: es-ES
ms.lasthandoff: 04/23/2019
ms.locfileid: "32283313"
---
# <a name="ado-md-api-reference"></a>Referencia de API de ADO MD

**Se aplica a:** Access 2013, Office 2013

Esta sección de la documentación de ActiveX Data Objects (ADO) MD contiene temas para cada objeto, colección, método y propiedad de ADO MD, así como código de ejemplo cuando corresponda. Para obtener más información, busque un tema específico en el índice o vea los temas siguientes:

- [Objetos de ADO MD](ado-md-objects.md)
- [Colecciones de ADO MD](ado-md-collections.md)
- [Propiedades de ADO MD](ado-md-properties.md)
- [Métodos de ADO MD](ado-md-methods.md)
- [Constantes enumeradas de ADO MD](ado-md-enumerated-constants.md)
- [Ejemplos de ADO MD](https://docs.microsoft.com/office/vba/access/concepts/miscellaneous/ado-md-code-examples)

