---
title: Objeto de conjunto de celdas (referencia de ADO MD)
TOCTitle: Cellset object (ADO MD)
ms:assetid: 0118476d-8cb6-49ca-80ac-5ace7987c878
ms:mtpsurl: https://msdn.microsoft.com/library/JJ248780(v=office.15)
ms:contentKeyID: 48542923
ms.date: 09/18/2015
mtps_version: v=office.15
localization_priority: Normal
ms.openlocfilehash: f44912d153b08cde57b742d4260b4d10746675b9
ms.sourcegitcommit: d6695c94415fa47952ee7961a69660abc0904434
ms.translationtype: Auto
ms.contentlocale: es-ES
ms.lasthandoff: 01/17/2019
ms.locfileid: "28720901"
---
# <a name="cellset-object-ado-md-reference"></a>Objeto de conjunto de celdas (referencia de ADO MD)

**Se aplica a**: Access 2013, Office 2013

- [Cellset (objeto, ADO MD)](cellset-object-ado-md.md)
- [Conjunto de celdas propiedades, métodos y eventos (ADO MD)](cellset-properties-methods-and-events-ado-md.md)

