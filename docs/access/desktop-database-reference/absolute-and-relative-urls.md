...
Una dirección URL especifica la ubicación de un destino almacenado en un equipo local o en red, como un archivo, un directorio, una página HTML, una imagen, un programa, entre otros. En esta discusión, *una dirección URL absoluta* tiene el formato:
...
|*resource*|Si se incluye, el *recurso* es el destino, que suele ser el nombre de un archivo. Puede ser un archivo *simple,* que contiene un único flujo binario de bytes, o un documento *estructurado,* que contiene uno o más almacenamientos y secuencias binarias de bytes.|
...
Se puede especificar una dirección URL absoluta que define un contexto en el parámetro *ActiveConnection* del método [Open](open-method-ado-record.md) del objeto **Record**. También se puede especificar una dirección URL absoluta como el valor de la palabra clave nueva en el parámetro Connection del objeto Open del método ConnectionString y el parámetro ActiveConnection del método Open del objeto `URL=` [Recordset.](recordset-object-ado.md)  [](open-method-ado-connection.md)  [](open-method-ado-recordset.md) 
...
El campo opciones indica que el tipo de comando es una dirección URL relativa.
...
## <a name="ole-db-provider-supplied-url-schemes"></a>Esquemas de dirección URL proporcionados por el proveedor OLE DB
...
ADO supports OLE DB providers that recognize their own URL schemes. Por ejemplo, el proveedor [microsoft OLE DB](microsoft-ole-db-provider-for-internet-publishing.md)para publicación en Internet , que tiene acceso a archivos "publicados" Windows 2000, reconoce el esquema HTTP existente.
