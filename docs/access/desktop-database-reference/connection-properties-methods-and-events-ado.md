---
title: Propiedades, métodos y eventos de Connection (ADO)
TOCTitle: Properties, Methods, and Events
ms:assetid: e78329a4-0b90-9ae5-f3d7-e56815a396fd
ms:mtpsurl: https://msdn.microsoft.com/library/JJ250174(v=office.15)
ms:contentKeyID: 48548407
ms.date: 09/18/2015
mtps_version: v=office.15
localization_priority: Normal
ms.openlocfilehash: e1fa956a5045e9678c45c74a160e3bb80e78e627
ms.sourcegitcommit: 8fe462c32b91c87911942c188f3445e85a54137c
ms.translationtype: MT
ms.contentlocale: es-ES
ms.lasthandoff: 04/23/2019
ms.locfileid: "32295845"
---
# <a name="connection-properties-methods-and-events-ado"></a>Propiedades, métodos y eventos de Connection (ADO)

**Se aplica a:** Access 2013, Office 2013

## <a name="propertiescollections"></a>Propiedades y colecciones

- [Attributes (propiedad)](attributes-property-ado.md)
- [CommandTimeout (propiedad)](commandtimeout-property-ado.md)
- [ConnectionString (propiedad)](connectionstring-property-ado.md)
- [ConnectionTimeout (propiedad)](connectiontimeout-property-ado.md)
- [CursorLocation (propiedad)](cursorlocation-property-ado.md)
- [DefaultDatabase (propiedad)](defaultdatabase-property-ado.md)
- [Errors (colección)](errors-collection-ado.md)
- [IsolationLevel (propiedad)](isolationlevel-property-ado.md)
- [Mode (propiedad)](mode-property-ado.md)
- [Properties (colección)](properties-collection-ado.md)
- [Provider (propiedad)](provider-property-ado.md)
- [State (propiedad)](state-property-ado.md)
- [Version (propiedad)](version-property-ado.md)


## <a name="methods"></a>Métodos

- [BeginTrans, CommitTrans y RollbackTrans (métodos)](begintrans-committrans-and-rollbacktrans-methods-ado.md)
- [Cancel (método)](cancel-method-ado.md)
- [Close (método)](close-method-ado.md)
- [Método execute (Connection de ADO)](https://docs.microsoft.com/office/vba/access/concepts/miscellaneous/execute-method-ado-connection)
- [Open (método, Connection de ADO)](open-method-ado-connection.md)
- [OpenSchema (método)](openschema-method-ado.md)


## <a name="events"></a>Eventos

- [BeginTransComplete, CommitTransComplete y RollbackTransComplete (eventos)](begintranscomplete-committranscomplete-and-rollbacktranscomplete-events-ado.md)
- [ConnectComplete y Disconnect (eventos)](connectcomplete-and-disconnect-events-ado.md)
- [ExecuteComplete (evento)](executecomplete-event-ado.md)
- [InfoMessage (evento)](infomessage-event-ado.md)
- [WillConnect (evento)](willconnect-event-ado.md)
- [WillExecute (evento)](willexecute-event-ado.md)


