...
Use el ejemplo siguiente en una página Active Server (ASP). Use **Buscar** para localizar el archivo Adovbs.inc y colóquelo en el directorio que desee utilizar. Corte y pegue el código siguiente en el Bloc de notas u otro editor de texto y guárdelo como **AddNewVBS.asp**. Podrá ver el resultado en cualquier explorador de cliente.

Para practicar con el ejemplo, agregue un registro nuevo en el formulario HTML. Haga clic en **Agregar nuevo**. Vea el [Ejemplo del método Delete](delete-method-example-vbscript.md) para quitar los registros no deseados.
