---
title: API exportadas de Outlook
manager: soliver
ms.date: 11/16/2014
ms.audience: Developer
ms.topic: overview
localization_priority: Normal
ms.assetid: a0fd2123-7c7a-40f3-8249-af2c9e9f272c
description: En esta sección se proporciona contenido de referencia para las API que exporta Outlook.
ms.openlocfilehash: 9c8adb8d23dfc44bf739cd5b980204611fb4dd31
ms.sourcegitcommit: 8657170d071f9bcf680aba50b9c07f2a4fb82283
ms.translationtype: MT
ms.contentlocale: es-ES
ms.lasthandoff: 04/28/2019
ms.locfileid: "33434315"
---
# <a name="outlook-exported-apis"></a>API exportadas de Outlook

En esta sección se proporciona contenido de referencia para las API que exporta Outlook.

- [Constants](constants-outlook-exported-apis.md)
    
- [Tipos de datos](data-types-outlook-exported-apis.md)
    
- [Functions](functions-outlook-exported-apis.md)
    
- [Interfaces](interfaces-outlook-exported-apis.md)
    
- [Los eventos disponibles y su identificadores DispId (API exportadas de Outlook)](available-events-and-their-dispids-outlook-exported-apis.md)
    

