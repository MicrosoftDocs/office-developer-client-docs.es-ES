...
Representa una [ACCT_BIN](acct_bin.md) que contiene el UID de una cuenta de Exchange. 
...
Use [PROP_ACCT_IS_EXCH](prop_acct_is_exch.md) para comprobar si la cuenta es una cuenta de Exchange. Si es así, **prop \_ MAPI_EMSMDB_UID** es un **ACCT_BIN** que contiene **el emsmdbUID**, que es el identificador único, para la cuenta de Exchange. Si la cuenta no es una cuenta de Exchange, esta propiedad no está definida.
