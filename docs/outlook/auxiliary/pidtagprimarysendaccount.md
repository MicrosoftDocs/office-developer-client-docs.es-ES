...
Esta propiedad se aplica a un objeto de mensaje MAPI. En el caso de un mensaje recibido, la marca "enviar" de la cuenta principal indica con qué cuenta se debe enviar un reenvío o una respuesta. Para un mensaje saliente, determina con qué cuenta enviar el mensaje. Su valor es el [PROP_ACCT_SEND_STAMP](prop_acct_send_stamp.md) de la [interfaz IOlkAccount](iolkaccount.md) de la cuenta con la que se envía el mensaje. 
