...
---
# <a name="create-a-helper-class-to-access-common-outlook-item-members"></a><span data-ttu-id="ee84f-102">Crear una clase Helper para acceder a los miembros comunes de elemento de Outlook</span><span class="sxs-lookup"><span data-stu-id="ee84f-102">Create a Helper class to access common Outlook item members</span></span>

<span data-ttu-id="ee84f-103">En este ejemplo se muestra cómo implementar una clase auxiliar de OutlookItem que tenga acceso a propiedades y métodos comunes de objetos de elementos de Outlook, ahorrando la sobrecarga de pruebas y convirtiendo a un elemento específico de objeto antes de tener acceso a estos miembros de elementos comunes.</span><span class="sxs-lookup"><span data-stu-id="ee84f-103">This example shows how to implement an OutlookItem helper class that accesses common properties and methods of Outlook item objects, saving the overhead of testing for and casting to a specific item object before accessing these common item members.</span></span>

## <a name="example"></a><span data-ttu-id="ee84f-104">Ejemplo</span><span class="sxs-lookup"><span data-stu-id="ee84f-104">Example</span></span>

> [!NOTE] 
> <span data-ttu-id="ee84f-105">El siguiente ejemplo de código es un fragmento de [Aplicaciones de programación para Microsoft Office Outlook 2007](https://www.amazon.com/gp/product/0735622493?ie=UTF8&tag=msmsdn-20&linkCode=as2&camp=1789&creative=9325&creativeASIN=0735622493).</span><span class="sxs-lookup"><span data-stu-id="ee84f-105">The following code example is an excerpt from [Programming Applications for Microsoft Office Outlook 2007](https://www.amazon.com/gp/product/0735622493?ie=UTF8&tag=msmsdn-20&linkCode=as2&camp=1789&creative=9325&creativeASIN=0735622493).</span></span>


<span data-ttu-id="ee84f-106">Muchos elementos de Outlook tienen métodos y propiedades similares.</span><span class="sxs-lookup"><span data-stu-id="ee84f-106">Many Outlook items have similar properties and methods.</span></span> <span data-ttu-id="ee84f-107">Por ejemplo, las propiedades **Application**, **Attachments**, **Body**, **Categories**, y **Class** y los métodos **Close**, **Copy**, y **Display** son comunes para todos los objetos de elementos de Outlook.</span><span class="sxs-lookup"><span data-stu-id="ee84f-107">For example, the **Application**, **Attachments**, **Body**, **Categories**, and **Class** properties, and **Close**, **Copy**, and **Display** methods are common to all Outlook item objects.</span></span> <span data-ttu-id="ee84f-108">El modelo de objetos de Outlook basados en COM devuelve el **Object** genérico de Visual Basic en lugar del tipo de elemento exacto de muchos miembros.</span><span class="sxs-lookup"><span data-stu-id="ee84f-108">The COM-based Outlook object model returns the generic Visual Basic **Object** instead of the exact item type for many members.</span></span> 

<span data-ttu-id="ee84f-109">Por ejemplo, la propiedad [CurrentItem](https://msdn.microsoft.com/library/bb611743\(v=office.15\)) devuelve un **Object** genérico.</span><span class="sxs-lookup"><span data-stu-id="ee84f-109">For example, the [CurrentItem](https://msdn.microsoft.com/library/bb611743\(v=office.15\)) property returns a generic **Object**.</span></span> <span data-ttu-id="ee84f-110">Por otro lado, el entorno de código administrado con establecimiento inflexible de tipos exige convertir el **Object** que representa un elemento de Outlook en el tipo exacto de Outlook como el **MailItem**.</span><span class="sxs-lookup"><span data-stu-id="ee84f-110">On the other hand, the strongly typed managed code environment requires you to cast the **Object** representing an Outlook item to the exact Outlook type such as **MailItem**.</span></span> 

<span data-ttu-id="ee84f-111">La clase auxiliar de OutlookItem usa la reflexión para mostrar las propiedades y los métodos que son comunes para todos los elementos.</span><span class="sxs-lookup"><span data-stu-id="ee84f-111">The OutlookItem helper class uses reflection to expose properties and methods that are common to all items.</span></span> <span data-ttu-id="ee84f-112">La clase ayuda a convertir el objeto en el tipo exacto y ofrece la comodidad de usar directamente los métodos o las propiedades de los elementos comunes del objeto **OutlookItem**.</span><span class="sxs-lookup"><span data-stu-id="ee84f-112">The class helps you cast the object to the exact type and provides the convenience of directly using common item properties or methods on the **OutlookItem** object.</span></span> <span data-ttu-id="ee84f-113">Esta es una técnica muy útil y varios otros temas de procedimientos que aparecen en la sección **Vea también** aprovechan esta clase auxiliar.</span><span class="sxs-lookup"><span data-stu-id="ee84f-113">This is a very useful technique, and several other how-to topics listed in the **See also** section take advantage of this helper class.</span></span>

<span data-ttu-id="ee84f-114">Si usa Visual Studio para probar este ejemplo de código, primero debe agregar una referencia al componente de la biblioteca de objetos de Microsoft Outlook 15.0 y especificar la variable de Outlook al importar el espacio de nombres **Microsoft.Office.Interop.Outlook**.</span><span class="sxs-lookup"><span data-stu-id="ee84f-114">If you use Visual Studio to test this code example, you must first add a reference to the Microsoft Outlook 15.0 Object Library component and specify the Outlook variable when you import the **Microsoft.Office.Interop.Outlook** namespace.</span></span> <span data-ttu-id="ee84f-115">La instrucción **Imports** o **using** no debe producirse directamente antes de las funciones en el ejemplo de código, pero deben agregarse antes de la declaración de clase pública.</span><span class="sxs-lookup"><span data-stu-id="ee84f-115">The **Imports** or **using** statement must not occur directly before the functions in the code example but must be added before the public Class declaration.</span></span> <span data-ttu-id="ee84f-116">La siguiente línea de código muestra cómo llevar a cabo la importación y la asignación en Visual Basic y C\#.</span><span class="sxs-lookup"><span data-stu-id="ee84f-116">The following lines of code show how to do the import and assignment in Visual Basic and C\#.</span></span>
...
## <a name="see-also"></a><span data-ttu-id="ee84f-117">Vea también</span><span class="sxs-lookup"><span data-stu-id="ee84f-117">See also</span></span>

- [<span data-ttu-id="ee84f-118">Mostrar elementos seleccionados en el Explorer activo</span><span class="sxs-lookup"><span data-stu-id="ee84f-118">Display selected items in the active Explorer</span></span>](how-to-display-selected-items-in-the-active-explorer.md)
- [<span data-ttu-id="ee84f-119">Abrir y ver los contenidos de un archivo de iCalendar</span><span class="sxs-lookup"><span data-stu-id="ee84f-119">Open and display the contents of an iCalendar file</span></span>](how-to-open-and-display-the-contents-of-an-icalendar-file.md)
- [<span data-ttu-id="ee84f-120">Asignar categorías a un elemento</span><span class="sxs-lookup"><span data-stu-id="ee84f-120">Assign categories to an item</span></span>](how-to-assign-categories-to-an-item.md)
- [<span data-ttu-id="ee84f-121">Implementar un contenedor para inspectores y realizar un seguimiento de eventos a nivel del elemento en cada inspector</span><span class="sxs-lookup"><span data-stu-id="ee84f-121">Implement a wrapper for inspectors and track item-level events in each inspector</span></span>](how-to-implement-a-wrapper-for-inspectors-and-track-item-level-events-in-each-inspector.md)
- [<span data-ttu-id="ee84f-122">Elementos generales de Outlook</span><span class="sxs-lookup"><span data-stu-id="ee84f-122">General Outlook items</span></span>](general-outlook-items.md)
