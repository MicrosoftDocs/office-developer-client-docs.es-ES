...
Si el buzón de correo está hospedado en un servidor de Exchange, se puede ejecutar una regla en el servidor de Exchange o en el cliente de Outlook. Si la regla se ejecuta en el servidor, no es necesario que Outlook esté en ejecución para que se evalúen las condiciones de la regla y se completen las acciones de esta. Si la regla se ejecuta en el cliente, Outlook debe estar ejecutándose para que se ejecute la regla. Si la propiedad [IsLocalRule](https://msdn.microsoft.com/library/bb647386\(v=office.15\)) del objeto [Rule](https://msdn.microsoft.com/library/bb647152\(v=office.15\)) devuelve **true**, la regla se ejecuta en el cliente.
