...
The MAPICODE. El archivo de encabezado H contiene muchos de los valores que un cliente o proveedor de servicios puede devolver de una implementación de método de interfaz o puede que se devuelvan de una llamada.
  
Los códigos para representar las condiciones de advertencia y error siguen una convención de nomenclatura diferente que comienza con el prefijo MAPI, un carácter de subrayado y una W o una E para indicar el tipo de código. El resto del código es una cadena de caracteres corta para describir la condición. Cada palabra de la cadena está separada por un carácter de subrayado. Por ejemplo, el valor de error MAPI_E_TOO_COMPLEX indica que la implementación no pudo controlar lo que se solicitaba en la llamada. El valor de advertencia MAPI_W_PARTIAL_COMPLETION indica que la llamada se ha hecho correctamente, pero que hubo problemas. Solo una parte de la operación se completó correctamente.
