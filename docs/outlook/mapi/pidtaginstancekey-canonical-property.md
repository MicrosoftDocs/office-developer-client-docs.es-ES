...
Cuando se expande una propiedad multivalor en una tabla, se crea una fila para cada instancia de la expansión, es decir, para cada valor de esa propiedad. Cada fila tiene un valor único para la **propiedad PR_INSTANCE_KEY,** mientras que todas las demás columnas conservan sus valores originales a lo largo de la expansión. 
  
En un tipo categorizado de una tabla, las filas que no corresponden a datos reales se pueden agregar al resultado de la ordenación. Cada fila de este tipo, como todas las filas de todas las tablas, tiene su propia clave de instancia única. 
