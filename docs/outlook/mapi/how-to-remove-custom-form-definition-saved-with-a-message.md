...
En este tema se muestra un ejemplo de código en C++ que convierte un mensaje que se ha guardado con una definición de formulario personalizada en un mensaje normal sin la definición de formulario.
  
En Microsoft Outlook 2010 o Microsoft Outlook 2013, puede compartir formularios que contengan páginas de formulario personalizadas publicándolos en una biblioteca de formularios o guardando la definición de formulario correspondiente con un mensaje. Un formulario personalizado guardado con un mensaje se conoce comúnmente como "formulario de uso único", ya que el formulario se comparte solo para ver ese mensaje específico como una instancia de uso único. Normalmente, esto se hace cuando el formulario no se publica en una biblioteca de formularios, pero se desea que el destinatario use el formulario personalizado al abrir el elemento. Puede especificar un formulario de uso único en el Diseñador  de formularios, seleccionando  la casilla Enviar definición de formulario con elemento en la página Propiedades del formulario. 
  
Los formularios que contienen páginas de formulario se pueden personalizar con código en Visual Basic Scripting Edition (VBScript). Los mensajes que se guardan con definiciones de formulario generalmente tienen un tamaño mayor. Por motivos de seguridad y almacenamiento, Outlook 2010 y Outlook 2013 omiten las definiciones de formulario guardadas con cualquier elemento.
  
Para convertir un mensaje que se guarda con una definición de formulario personalizada en uno sin, debe quitar cuatro propiedades con nombre:
...
La siguiente función, , acepta como parámetros de entrada un puntero a un mensaje y un indicador de si se quita la propiedad canónica  `RemoveOneOff` [PidLidPropertyDefinitionStream](pidlidpropertydefinitionstream-canonical-property.md). Mediante el puntero del mensaje, llama a [IMAPIProp::GetIDsFromNames](imapiprop-getidsfromnames.md) para obtener los identificadores de propiedad adecuados y, a continuación, llama a [IMAPIProp::D eleteProps](imapiprop-deleteprops.md) para quitar las propiedades con nombre. También llama a [IMAPIProp::GetProps](imapiprop-getprops.md) para obtener la propiedad canónica [PidLidCustomFlag](pidlidcustomflag-canonical-property.md) y borra la marca **\_ ONEOFFFLAGS de INSP** y la marca INSP_PROPDEFINITION según corresponda **de** esa propiedad, para que Outlook 2010 y Outlook 2013 no busquen las propiedades con nombre que se han quitado. 
