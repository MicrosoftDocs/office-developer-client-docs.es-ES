...
Contiene la dirección de correo electrónico del Protocolo simple de transferencia de correo (SMTP) del usuario de mensajería representado por el remitente.
...
Esta propiedad es un ejemplo de las propiedades de dirección para el usuario de correo que representa el remitente. Cuando una aplicación cliente envía un mensaje en nombre de otro cliente, debe establecer todas las propiedades de remitente representadas en los valores de ese cliente. Un usuario de mensajería que envía por su cuenta normalmente deja las propiedades de remitente representadas no establecidas.
  
El proveedor de transporte de salida siempre debe dejar esta propiedad sin cambios si el cliente de envío lo ha establecido. Si está desactivada, el proveedor de transporte debe establecerla en la propiedad **PR_SENDER_SMTP_ADDRESS** ([PidTagSenderSmtpAddress](pidtagsendersmtpaddress-canonical-property.md)) en la copia de salida del mensaje y dejarla sin establecer en la copia local.
