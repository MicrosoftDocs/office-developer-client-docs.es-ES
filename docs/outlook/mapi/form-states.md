...
Los objetos form pueden encontrarse en uno de los cinco estados distintos, en función de los métodos que se hayan llamado en ellos y de si se han producido errores al realizar dichos métodos. Los estados se describen en los siguientes temas:
  
- [Estado no inicializado](uninitialized-state.md)
    
- [Estado normal](normal-state.md)
    
- [Estado nocribible](noscribble-state.md)
    
- [Estado de HandsOffAfterSave](handsoffaftersave-state.md)
    
- [Estado HandsOffFromNormal](handsofffromnormal-state.md)
    
Los estados se relacionan principalmente con el estado de los datos en el objeto de formulario. Los distintos estados reflejan si los datos deben guardarse, si el objeto de formulario debe permitir modificaciones en los datos y en qué punto del proceso de guardar los datos en los que se encuentra el formulario. Por lo tanto, los estados de formulario y las transiciones entre ellos tienen más que ver con la implementación del servidor de formulario de los métodos de interfaz [IPersistMessage : IUnknown](ipersistmessageiunknown.md) que cualquier otro. El conocimiento de estos estados es muy útil para la correcta implementación de las interfaces de formulario MAPI que debe implementar el servidor de formularios. 
