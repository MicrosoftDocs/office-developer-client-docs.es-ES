...
2. <span data-ttu-id="ede34-128">Llamar al método [IUnknown:: AddRef](http://msdn.microsoft.com/es-es/library/ms691379%28VS.85%29.aspx) de su formulario para que el almacenamiento de información para el formulario y las estructuras de datos internos permanecen disponibles hasta que haya terminado el procesamiento.</span><span class="sxs-lookup"><span data-stu-id="ede34-128">Call your form's [IUnknown::AddRef](http://msdn.microsoft.com/es-es/library/ms691379%28VS.85%29.aspx) method so that storage for the form and any internal data structures remain available until processing is finished.</span></span> 
...
5. <span data-ttu-id="ede34-132">Liberar el formulario mensaje y objetos de sitio de mensaje llamando a sus métodos [IUnknown:: Release](http://msdn.microsoft.com/es-es/library/ms682317%28v=VS.85%29.aspx) .</span><span class="sxs-lookup"><span data-stu-id="ede34-132">Release your form's message and message site objects by calling their [IUnknown::Release](http://msdn.microsoft.com/es-es/library/ms682317%28v=VS.85%29.aspx) methods.</span></span> 
...
> [!NOTE]
> <span data-ttu-id="ede34-138">Después de que se hayan completado estas acciones, los métodos solo es válidos en el objeto de formulario que se puede llamar son los de la interfaz [IUnknown](http://msdn.microsoft.com/es-es/library/ms680509%28v=VS.85%29.aspx) .</span><span class="sxs-lookup"><span data-stu-id="ede34-138">After these actions have been completed, the only valid methods on the form object that may be called are those from the [IUnknown](http://msdn.microsoft.com/es-es/library/ms680509%28v=VS.85%29.aspx) interface.</span></span> 
