...
Las aplicaciones cliente y los proveedores de servicios llaman al **método GetDefaultDir** para recuperar el identificador de entrada del contenedor de libreta de direcciones predeterminado. El contenedor predeterminado es lo que el usuario ve que se muestra en la libreta de direcciones cuando se abre por primera vez la libreta de direcciones. Si no se ha establecido un contenedor predeterminado mediante una llamada al método [IAddrBook::SetDefaultDir,](iaddrbook-setdefaultdir.md) MAPI asigna como contenedor predeterminado el primer contenedor con nombres que no son la libreta de direcciones personal (PAB). Si no se encuentra un contenedor de este tipo, el PAB se convierte en el contenedor predeterminado. 
