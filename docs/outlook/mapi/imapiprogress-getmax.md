...
El valor de la configuración de la marca afecta a si el objeto de progreso comprende que el valor máximo sea local o global. Cuando se MAPI_TOP_LEVEL marca, el valor máximo se considera global y se usa para calcular el progreso de toda la operación. Cuando MAPI_TOP_LEVEL no se establece, el valor máximo se considera local y los proveedores lo usan internamente para mostrar el progreso de los subobjetos de nivel inferior. Los objetos de progreso guarda el valor máximo local solo para devolverlo a un proveedor a través de una **llamada GetMax.** 
