...
Para obtener acceso a la tabla de uso único que pertenece a uno de los contenedores, MAPI llama al método [IMAPIProp::OpenProperty](imapiprop-openproperty.md) del contenedor para abrir la propiedad **PR_CREATE_TEMPLATES** ([PidTagCreateTemplates](pidtagcreatetemplates-canonical-property.md)) con la interfaz **IMAPITable.** Se le pide al contenedor que devuelva su tabla de uso único cuando una aplicación cliente intente agregar un destinatario al contenedor. Si el contenedor permite a algún destinatario, el proveedor puede devolver su propia implementación de tabla o llamar a [IMAPISupport::GetOneOffTable](imapisupport-getoneofftable.md) para devolver la implementación MAPI. 
